...
def test_create_approval_success(client, db_session, test_user, pending_status, access_token):
    payload = {
        "title": "モニター購入",
        "amount": 50000,
        "reason": "業務効率化のため",
    ...
    approval = db_session.scalar(select(PurchasingApproval).where(PurchasingApproval.title == "モニター購入"))
    assert approval is not None
    assert approval.applicant_user_id == test_user.id
    assert approval.amount == 50000
